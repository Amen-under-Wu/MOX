...
trait DiffEq {
    fn derivative(&self, x: Float, y: &MyVec) -> MyVec;
    fn jacobian(&self, x: Float, y: &MyVec) -> Jacobian;
}
...
trait Stepper {
    ...
    fn dense_out(&self, x: Float, h: Float) -> MyVec;
...
impl Controller853 {
    ...
    fn success(&mut self, err: Float, h: &mut Float) -> bool {
        let mut scale: Float;
        if err <= 1.0 {
            if err == 0.0 {
...
impl Output {
    ...
    fn save(&mut self, x: Float, y: &MyVec) {
...
impl StepperRoss {
    fn prepare_step(
        ...
        a.gauss_j(&mut Matrix::new(0, 0))?;
        let y_temp = &stat.dy_dx + h * Self::d1 * &stat.jacobian.df_dx;
        self.k1 = a.mul_vec(&y_temp);
        ...
        self.k6 = dydx_new
            + (1.0 / h)
                * (Self::c61 * &self.k1
                    ...
                    + Self::c64 * &self.k4
                    + Self::c65 * &self.k5);
        self.data.y_err = a.mul_vec(&self.k6);
        self.data.y_out = &self.data.y_err + y_temp;
        Ok(())
    }
    fn prepare_dense(&mut self, stat: &ProbStat, h: Float, dydx_new: &MyVec) {
...
impl Stepper for StepperRoss {
    ...
    fn step(
        ...
        if self.data.dense {
            self.prepare_dense(stat, h, &dydx_new);
        }
        stat.dy_dx = dydx_new;
        stat.y = self.data.y_out.clone();
        self.data.x_old = stat.x;
