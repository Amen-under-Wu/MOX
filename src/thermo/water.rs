...
const R: Float = 461.526; // J/(kg*K)
const T_C: Float = 647.096; // K
const P_C: Float = 22.064e6; // Pa
const RHO_C: Float = 322.0; // kg/m^3

const TABLE1: [Float; 5] = [
    0.34805185628969e3,
    -1.1671859879975,
    0.10192970039326e-2,
    0.57254459862746e3,
...
fn eq5(t: Float) -> Float {
    1e6 * (TABLE1[0] + TABLE1[1] * t + TABLE1[2] * t * t)
}
fn eq6(p: Float) -> Float {
    let pi = p * 1e-6;
    TABLE1[3] + ((pi - TABLE1[4]) / TABLE1[2]).sqrt()
}

const TABLE2: [(i32, i32, Float); 34] = [
...
fn eq7(pi: Float, tau: Float) -> Float {
    TABLE2
        .iter()
        .map(|x| x.2 * (7.1 - pi).powi(x.0) * (tau - 1.222).powi(x.1))
        .sum()
}
fn eq7_dgdt(pi: Float, tau: Float) -> Float {
    TABLE2
        .iter()
        .map(|x| x.2 * (7.1 - pi).powi(x.0) * x.1 as Float * (tau - 1.222).powi(x.1 - 1))
        .sum()
}
const P_STAR_EQ7: Float = 16.53e6;
const T_STAR_EQ7: Float = 1386.0;
fn r1_spt(p: Float, t: Float) -> Float {
    let pi = p / P_STAR_EQ7;
    let tau = T_STAR_EQ7 / t;
    R * (tau * eq7_dgdt(pi, tau) - eq7(pi, tau))
}
fn r1_hpt(p: Float, t: Float) -> Float {
    let pi = p / P_STAR_EQ7;
    let tau = T_STAR_EQ7 / t;
    R * t * tau * eq7_dgdt(pi, tau)
}
const TABLE6: [(i32, i32, Float); 20] = [
...
fn r1_tph(p: Float, h: Float) -> Float {
    let pi = p * 1e-6;
    let eta = h * 4e-7;
    TABLE6
        .iter()
        .map(|x| x.2 * pi.powi(x.0) * (eta + 1.0).powi(x.1))
        .sum()
}

const TABLE7: [(i32, i32, Float); 20] = [
...
fn r1_tps(p: Float, s: Float) -> Float {
    let pi = p * 1e-6;
    let sigma = s * 1e-3;
    TABLE7
        .iter()
        .map(|x| x.2 * pi.powi(x.0) * (sigma + 2.0).powi(x.1))
        .sum()
}
const TABLE10: [(i32, Float); 9] = [
...
fn eq16(pi: Float, tau: Float) -> Float {
    pi.ln() + TABLE10.iter().map(|x| x.1 * tau.powi(x.0)).sum::<Float>()
}

const TABLE11: [(i32, i32, Float); 43] = [
...
fn eq17(pi: Float, tau: Float) -> Float {
    TABLE11
        .iter()
        .map(|x| x.2 * pi.powi(x.0) * (tau - 0.5).powi(x.1))
        .sum()
}
fn eq15(pi: Float, tau: Float) -> Float {
    eq16(pi, tau) + eq17(pi, tau)
}
fn eq15_dgdt(pi: Float, tau: Float) -> Float {
    TABLE11
        .iter()
        .map(|x| x.2 * pi.powi(x.0) * x.1 as Float * (tau - 0.5).powi(x.1 - 1))
        .sum::<Float>()
        + TABLE10
            .iter()
            .map(|x| x.0 as Float * x.1 * tau.powi(x.0 - 1))
            .sum::<Float>()
}
fn r2_spt(p: Float, t: Float) -> Float {
    let pi = p * 1e-6;
    let tau = 540.0 / t;
    R * (tau * eq15_dgdt(pi, tau) - eq15(pi, tau))
}
fn r2_hpt(p: Float, t: Float) -> Float {
    let pi = p * 1e-6;
    let tau = 540.0 / t;
    R * t * tau * eq15_dgdt(pi, tau)
}
/*
fn get_region(p: Float, t: Float) -> Option<Region> {
    if p < 0.0 || t < 273.15 {
        return None;
    }
    if t > 1073.15 && t <= 2273.15 && p <= 10e6 {
        Some(Region::R5)
    } else if t <= 1073.15 && p <= 100e6 {
        if t > 623.15 && t <= eq6(p) {
            Some(Region::R3)
        } else
    } else {
...
mod tests {
    ...
    #[test]
    fn test_eq5_eq6() {
        const T: Float = 623.15;
        const P: Float = 0.165291643e8;
        assert!((eq5(T) - P).abs() / P < 1e-8);
        assert!((eq6(P) - T).abs() / T < 1e-8);
    }
    #[test]
    fn test_eq7() {
        const TABLE5: [(Float, Float, Float, Float); 3] = [
            (300.0, 3e6, 0.115331273e6, 0.392294792e3),
            (300.0, 80e6, 0.184142828e6, 0.368563852e3),
            (500.0, 3e6, 0.975542239e6, 0.258041912e4),
        ];
        for (t, p, h, s) in TABLE5 {
            assert!((r1_hpt(p, t) - h).abs() / h < 1e-8);
            assert!((r1_spt(p, t) - s).abs() / s < 1e-8);
        }
    }
    #[test]
    fn test_eq11() {
        const TABLE7: [(Float, Float, Float); 3] = [
            (3e6, 500e3, 0.391798509e3),
            (80e6, 500e3, 0.378108626e3),
            (80e6, 1500e3, 0.611041229e3),
        ];
        for (p, h, t) in TABLE7 {
            assert!((r1_tph(p, h) - t).abs() / t < 1e-8);
        }
    }
    #[test]
    fn test_eq13() {
        const TABLE9: [(Float, Float, Float); 3] = [
            (3e6, 0.5e3, 0.307842258e3),
            (80e6, 0.5e3, 0.309979785e3),
            (80e6, 3e3, 0.565899909e3),
        ];
        for (p, s, t) in TABLE9 {
            assert!((r1_tps(p, s) - t).abs() / t < 1e-8);
        }
    }
    #[test]
    fn test_eq15() {
        const TABLE18: [(Float, Float, Float, Float); 3] = [
            (300.0, 0.0035e6, 0.254991145e7, 0.852238967e4),
            (700.0, 0.0035e6, 0.333568375e7, 0.101749996e5),
            (700.0, 30e6, 0.263149474e7, 0.517540298e4),
        ];
        for (t, p, h, s) in TABLE18 {
            assert!((r2_hpt(p, t) - h).abs() / h < 1e-8);
            assert!((r2_spt(p, t) - s).abs() / s < 1e-8);
        }
    }
}
